...
# recursive_tree_search: busca recursiva de um valor em árvore
# binária.
# Parâmetros: 
#   a0 - endereço de root_node
#   a1 - valor procurado
# Retorno:
#   a0 - nível do nó onde o valor encontra-se. Iniciando em 1,
# retorna 0 se não encontrar.
recursive_tree_search:
    addi sp, sp, -8     # Salvando conteúdo de RA e a0 atual
    sw ra, 4(sp)        # Salva o endereço de retorno
    sw a0, 0(sp)        # Salva o a0 atual (endereço do nó atual)

    # Vejo se cheguei em um nó NULL.
    beqz a0, 3f

    # 0: Vejo se o nó tem o valor. Se sim, retorno 1
    0:
    lw t0, (a0)
    bne t0, a1, 1f

    # Encontrou: não preciso recuperar a0, apenas ra.
    li a0, 1
    lw ra, 4(sp)
    addi sp, sp, 8
    ret

    # 1: Se o nó não tiver o valor, chamo a função na árvore esquerda
    1:
    lw a0, 4(a0)
    jal recursive_tree_search

    # Se tiver o valor, a0 != 0: valor encontrado.
    beqz a0, 2f
    ...
    # 2: Se ainda assim não for encontrado o valor, vou à árvore direita
    2:
    lw a0, (sp)
    lw a0, 8(a0)
    jal recursive_tree_search
    ...
    # 3: O valor não foi encontrado. Volto ao nó anterior, subindo na árvore
    3:
    li a0, 0
    lw ra, 4(sp)
    addi sp, sp, 8
