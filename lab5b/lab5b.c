...
int pack(int value, int lshift, int packed_value) {
    int shifted_value = value << lshift;
...
int get_type_and_pack(InstData *data) {
    int packed_value = data->opcode;

    if (data->type == R) {
        packed_value = pack(data->rd, 7, packed_value);
        packed_value = pack(data->funct3, 12, packed_value);
        packed_value = pack(data->rs1, 15, packed_value);
        packed_value = pack(data->rs2, 20, packed_value);
        packed_value = pack(data->funct7, 25, packed_value);
    }
    else if (data->type == I) {
        packed_value = pack(data->rd, 7, packed_value);
        packed_value = pack(data->funct3, 12, packed_value);
        packed_value = pack(data->rs1, 15, packed_value);
        packed_value = pack(data->imm, 20, packed_value);
    }
    else if (data->type == S) {
        packed_value = pack(mask(data->imm, 31), 7, packed_value);
        packed_value = pack(data->funct3, 12, packed_value);
        packed_value = pack(data->rs1, 15, packed_value);
        packed_value = pack(data->rs2, 20, packed_value);
        packed_value = pack(mask(data->imm, 4064), 20, packed_value);
    }
    else if (data->type == B) {
        packed_value = pack((mask(data->imm, 2048)) >> 4, 0, packed_value);
        packed_value = pack(mask(data->imm, 30), 7, packed_value);
        packed_value = pack(data->funct3, 12, packed_value);
        packed_value = pack(data->rs1, 15, packed_value);
        packed_value = pack(data->rs2, 20, packed_value);
        packed_value = pack(mask(data->imm, 2016), 20, packed_value);
        packed_value = pack(mask(data->imm, 4096), 19, packed_value);
    }
    else if (data->type == U) {
        packed_value = pack(data->rd, 7, packed_value);
        packed_value = pack(data->imm, 12, packed_value);
    }
    else if (data->type == J) {
        packed_value = pack(data->rd, 7, packed_value);
        packed_value = pack(mask(data->imm, 1044480), 0, packed_value);
        packed_value = pack(mask(data->imm, 2048), 9, packed_value);
        packed_value = pack(mask(data->imm, 2046), 20, packed_value);
        packed_value = pack(mask(data->imm, 1048576), 11, packed_value);
    }
...
int main() {
    ...
    int n = read(STDIN_FD, (void*) input_buffer, 40);

    InstData data;
    get_inst_data(input_buffer, &data);
    packed_value = get_type_and_pack(&data);
